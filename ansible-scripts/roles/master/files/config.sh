...
sed -i s/MESOS_ADVERTISE_IP=.*/MESOS_ADVERTISE_IP=${ip}/ ${work_dir}/docker-compose/master/mesos_master_env

sed -i s@MESOS_ZK=.*@MESOS_ZK=zk://${ip}:2181/mesos@ ${work_dir}/docker-compose/master/mesos_master_env
sed -i s/MESOS_HOSTNAME=.*/MESOS_HOSTNAME=${ad_ip}/ ${work_dir}/docker-compose/master/mesos_master_env
...
sed -i s@MARATHON_WEBUI_URL=.*@MARATHON_WEBUI_URL=${ad_ip}@ ${work_dir}/docker-compose/master/marathon_env
